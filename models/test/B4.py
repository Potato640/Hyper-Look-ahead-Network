...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, n_win=7, num_heads=8, qk_dim=None, qk_scale=None,
                 kv_per_win=4, kv_downsample_ratio=4, mlp_ratio=4,kv_downsample_kernel=None, kv_downsample_mode='identity',
                 topk=4, param_attention="qkvo", param_routing=False, diff_routing=False, soft_routing=False,
                 side_dwconv=3,norm_layer=nn.LayerNorm,act_layer=nn.SiLU,drop=0.,
                 auto_pad=True):
        super().__init__()
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        if ret_attn_mask:
            ...
            shortcut2 = shortcut1 + out
            # print("sh2",shortcut2.shape)
            B, H, W, C = shortcut2.shape
            L = H * W
            out = shortcut2.permute(0, 2, 3, 1).contiguous().view(B, L, C)  # b, L, c
            out = self.mlp(self.norm2(out))
            # print("fin",out.shape)
            out = out.view(B, H, W, C)
            # se = SEAttention(channel=self.dim, reduction=8)
